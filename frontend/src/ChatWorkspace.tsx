...
export function ChatWorkspace() {
  ...
  return <section><h1>workspace</h1><form onSubmit={submit}><label>Workspace<select required value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="">Select allowlisted workspace</option>{projects.data.projects.map((project) => <option key={project.project_id} value={project.project_id}>{project.display_name}</option>)}</select></label>{selectedProject ? <p>{selectedProject.file_count} files, {selectedProject.included_bytes} bytes; excludes {selectedProject.excluded_paths.join(", ") || "nothing"}.</p> : null}<label>Request<textarea required value={message} onChange={(event) => setMessage(event.target.value)} /></label><button disabled={!projectId || !message.trim()} type="submit">Start run</button></form>{error ? <p role="alert">{error}</p> : null}<section aria-label="Run history"><h2>Run history</h2><ul>{runs.data.map((run) => <li key={run.run_id}><button type="button" onClick={() => setActiveRun(run)}>{run.run_id}</button> — {run.status}</li>)}</ul></section>{activeRun ? <><Timeline runId={activeRun.run_id} /><WorkspaceControls runId={activeRun.run_id} /><PromotionControls runId={activeRun.run_id} /></> : null}</section>;
}
