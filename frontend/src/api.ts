...
export const promotionPreviewSchema = z.object({ preview_hash: z.string(), direct_eligible: z.boolean(), checkpoint_commit: z.string(), changed_files: z.array(z.string()), checks: z.array(z.object({ name: z.string(), status: z.enum(["passed", "failed", "pending"]) })), issues: z.array(z.string()), baseline: z.string(), proposed_version: z.string(), conflict_reason: z.string().nullable() });
export const promotionResultSchema = z.object({ status: z.enum(["committed", "fallback", "rejected"]), branch: z.string().nullable(), commit_hash: z.string().nullable(), tag: z.string().nullable(), reason: z.string().nullable() });
